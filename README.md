...
- **Document navigation + drafting** — `@teamsuzie/markdown-document` plus the sibling `markitdown-agent` (Python: MarkItDown + pypandoc). Upload a DOCX/PDF/PPTX; call `convert_to_markdown(file_id)` and the agent gets `get_outline` / `read_section` / `search_document` to answer questions. Or call `create_document` / `set_outline` / `write_section` / `export_to_docx` to draft a memo and ship a styled DOCX. Same primitive backs both flows.
